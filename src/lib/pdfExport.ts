...
export const generateInvestmentReport = async (
  ...
  // Add header
  const companyName = options.companyName || "PlansureAI";
  ...
  pdf.setFontSize(24);
  pdf.text("Investment Analysis Report", margin, 20);
  
  pdf.setFontSize(10);
  pdf.text(`Generated by ${companyName}`, margin, 28);
  ...
  if (options.includeComments && options.comments && options.comments.length > 0) {
    ...
    pdf.setFont(undefined, "bold");
    pdf.setFontSize(16);
    pdf.text("Team Comments & Annotations", margin, yPos);
    yPos += 10;
    ...
    options.comments.forEach((comment) => {
      ...
      // Comment content
      pdf.setFont(undefined, "normal");
      const commentLines = pdf.splitTextToSize(comment.content, pageWidth - 2 * margin);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    pdf.text(
      `Page ${i} of ${totalPages} | ${companyName} Investment Analysis`,
      pageWidth / 2,
      pageHeight - 10,
      { align: "center" }
    );
